...
- [x] **Value assignments as named constants** — `maxVolume INTEGER ::= 100` at module scope, usable afterward in any INTEGER or SIZE constraint bound (e.g. `INTEGER (0..maxVolume)`); resolved to a literal during parsing with zero wire-format impact
- [x] **`IMPORTS ... FROM`** — cross-module type references (e.g. `IMPORTS Track FROM PlaylistModule;`); the Maven plugin resolves the exporting module's Java package across all configured `specFile`s (including per-spec `packageName` overrides) and generates a correctly cross-package-qualified delegating codec; encodes identically to a same-module type reference
