...
# ── generate.sh ──────────────────────────────────────────────────────────────
#
# Builds a Docker image with asn1c and uses it to UPER-encode one or more
# JER (JSON) inputs against an ASN.1 spec, producing golden-test artifacts.
#
# Usage:
#   ./generate.sh --spec <file.asn> --type <RootType> --input <a.json> [--input <b.json> ...]
#
# Outputs (written to ../golden-tests/generated/):
...
#       --input ../examples/valid-1.json \
#       --input ../examples/valid-2.json \
#       --input ../examples/invalid-1.json
# ─────────────────────────────────────────────────────────────────────────────

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
SPEC_NAME=""  # resolved after argument parsing
IMAGE_NAME="asn1c-uper-encoder"
